			...
			unsigned long arg)
{
	...
	case FBIO_UI_OPEN: {
		if (!info->fbui) {
			ret = FBUI_ERR_NOT_OPERATIONAL;
			break;
		}

			if (access_ok ((char*) arg, sizeof(struct fbui_open))) {
			ret = fbui_open (info, (struct fbui_open*) arg);
			break;
			} else {
			ret = -EFAULT;
			break;
	...
	case FBIO_UI_CLOSE:
		...
		if (access_ok ((char*) arg, 4)) {
			...
			arg += 4;
			if (access_ok ((char*) (arg), 2*nwords)) {
				ret = fbui_exec (info, win_id, nwords, (unsigned char*) (arg));
				break;
			} else {
				ret = -EFAULT;
				break;
